...
int tmr_set_prescaler(int p, int  value)
{
	...
	rTCFG0 &= ~(0xFF << offset);//reinicia
	rTCFG0 |= (value << offset);//pone el nuevo valor 
...
int tmr_set_divider(int d, enum tmr_div div)
{
	...
	rTCFG1 &= ~(0xFF << pos);//reinicia el valor
	rTCFG1 |= (div << pos);//pone el nuevo valor
